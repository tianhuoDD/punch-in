<template>
  <div class="app-container">
    ...
    <van-nav-bar class="top-nav" style="background-color: #f5f5f5">
      ...
      <!-- 动态设置左侧按钮是否可见 -->
      <template #left v-if="navbarStore.leftButtonVisible">
        <van-icon name="arrow-left" @click="goBack" />
      </template>

      <!-- 动态设置右侧按钮是否可见 -->
      <template #right v-if="navbarStore.rightButtonVisible">
        <van-icon name="setting-o" @click="toggleActionSheet" />
      </template>
    </van-nav-bar>
    <!-- 下拉菜单 - action sheet -->
    <van-action-sheet v-model:show="showActionSheet" :actions="actions" close-on-click-action cancel-text="取消" @select="onSelectAction" />
    <!-- 实际页面内容 -->
    ...
    <van-tabbar v-model="active" route class="bottom-tabbar">
      <van-tabbar-item replace to="/PunchIn" icon="home-o" style="background-color: #f5f5f5 !important">打卡</van-tabbar-item>
      <van-tabbar-item replace to="/RecordBills" icon="search" style="background-color: #f5f5f5 !important">记账</van-tabbar-item>
    </van-tabbar>
  </div>
</template>
